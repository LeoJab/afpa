...
if($libelle == NULL || $active == NULL) {
    header('Location: /index.php?admin=modif_categorie');
    exit;
}
...
try {
    $requete = $db->prepare("UPDATE categorie SET libelle = :libelle, image = :image, active = :active where id = :id");
    ...
    die("Fin du script (script_modif_cate.php)");
}
